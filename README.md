...
## 1) Centralized & Distributed Version Control 	
...
h) git clone url	to clone the remote repository in your local system 
i) git rebase master 			
		
git rebase master will combine and move the present series of commmits from the point they have diverged from master and will put them in front of master and master will become their ancestor and the old series of commits will be left where they were.
...
## 10) git remote add new-remote-repo <remote-url-link>
...
If you forget to add a file in recently made commit or added wrong message then stage the files using git add and then make 
git commit --amend to make change in the last commit only without creating new commit
...
so the in such case the master is switched directly to the most recently added commit(in this case made by other branch).
...
If two diverged line of development represented by two different branches are merged and both have updated the same file and same 		piece or lines of code then merge conflict will occur and conflict resolution markers will be written in the file with HEAD section 	    containing the branch in which we are merging code(i.e, generally master.)
...
## 18) Remote tracking branches are local branches that mirror or track a remote branch and it's reference cannot be changed by user.
